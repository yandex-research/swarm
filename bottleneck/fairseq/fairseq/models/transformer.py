...
class TransformerModel(FairseqEncoderDecoderModel):
    ...
    def build_embedding(cls, args, dictionary, embed_dim, path=None):
        ...
        if not args.stable_emb:
            ...
            emb = bnb.nn.StableEmbedding(len(dictionary), embed_dim, dictionary.pad(), sparse=False)
...
class TransformerDecoder(FairseqIncrementalDecoder):
    ...
    def __init__(
        ...
        self.layers.extend(
            [
                self.build_decoder_layer(args, no_encoder_attn, index=i)
    ...
    def build_decoder_layer(self, args, no_encoder_attn=False, index=0):
        layer = TransformerDecoderLayer(args, no_encoder_attn, index=index)
        checkpoint = getattr(args, "checkpoint_activations", False)
        if checkpoint:
            offload_to_cpu = getattr(args, "offload_activations", False)
            layer = checkpoint_wrapper(layer, offload_to_cpu=offload_to_cpu)
