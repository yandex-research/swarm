...
from fairseq.modules.transformer_layer import FFBottleneck
from fairseq.modules.layer_norm import FusedLayerNorm, Fp32LayerNorm
    ...
    def __init__(self, cfg: FairseqConfig, task, model, criterion, quantizer=None):
        ...
        if cfg.distributed_training.ddp_backend != "fully_sharded":
            if cfg.common.fp16:
                ...
                #print('clamping...')
                #for name, parent in self._model.named_modules():
                ...
                #            if p.numel() > 4096:
                #                with torch.no_grad():
                #                    q = F.estimate_quantiles(p.cuda(), offset=0.01).cpu()
                #                    #p[p<q[20]] *= 0.8
                #                    #p[p>q[-20]] *= 0.8
                #                    p.clamp_(q[1].item(), q[-1].item())
                #            #else:
                #            #    with torch.no_grad():
                #            #        values, idx = torch.sort(p.flatten())
                #            #        n = p.numel()
                #            #        a = values[int(n*0.03)]
                #            #        b = values[int(n*0.97)]
                #            #        p.clamp_(a, b)
                ...
                for name, parent in self._model.named_modules():
                    for name, module in parent.named_children():
                        if isinstance(module, FFBottleneck):
                            module.norm1 = module.norm1.float()
                            module.norm2 = module.norm2.float()
                            if hasattr(module, 'fc1'):
                                module.fc1 = module.fc1.float()
                        if (isinstance(module, torch.nn.LayerNorm) or
                            isinstance(module, FusedLayerNorm)):
                            #print(module.weight.device)
                            norms.append((parent, name))


                #for parent, name in norms:
                #    module = getattr(parent, name)
                #    setattr(parent, name, Fp32LayerNorm(module.weight.shape[0]))
